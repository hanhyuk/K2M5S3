...
public class MapleGuild {
	...
	public boolean purchaseSkill(int skill, String name, int cid) {
		...
		// if (skillid.getReqGuildLevel() > getLevel() || skillid.getLevel() <=
		// getSkillLevel(skill)) {
		// return false;
		// }
	...
	// character with exceptionId) in all channels with as little access to rmi
	// as possible
	...
	public final int addGuildMember(final MapleGuildCharacter mgc, final MapleClient c) {
		// first of all, insert it into the members keeping alphabetical order
		// of lowest ranks ;)
		lock.lock();
		try {
