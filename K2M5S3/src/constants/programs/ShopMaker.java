...
public class ShopMaker {
	private static final Logger logger = LoggerFactory.getLogger(ShopMaker.class);
	...
	private static void dropInfo(List<Integer> list, PrintWriter name) {
		...
		for (Integer Int : list) {
			...
			String shopname;
			if (et == null)
				shopname = ShopMaker.MapleWeaponType.getMapleWeaponTypeById(value - ShopMaker.EquipType.무기.shopIdAlter()).name();
			else
				shopname = et.name();
			String a = "    ";
			a += shopname;
	...
	private static List<Triple<Integer, Integer, Integer>> getAllItems() { // 아이템,
																			// 상점,
																			// 가격
		...
		for (String cat : category) {
			...
			for (File file : path.listFiles()) {
				...
				// if (MapleDataTool.getInt("reqLevel", info, 0) > 100)
				// continue;
				// if (MapleDataTool.getInt("tradeBlock", info, -1) != -1)
				// continue;
				...
				isCash = MapleDataTool.getInt("cash", info, 0) == 1; // 캐시 아이템
																		// 여부 확인
				level = MapleDataTool.getInt("reqLevel", info, 0);
				/*
				 * if (level >= 10 && level < 20) { price = 10000; } else if
				 * (level >= 20 && level < 30) { price = 50000; } else if (level
				 * >= 30 && level < 40) { price = 100000; } else if (level >= 40
				 * && level < 50) { price = 200000; } else if (level >= 50 &&
				 * level < 60) { price = 500000; } else if (level >= 60 && level
				 * < 70) { price = 700000; } else if (level >= 70 && level < 71)
				 * { price = 1000000; }
				 * 
				 */
				...
				try {
					...
					if (!isCash) {
						...
						// ShopMaker.MapleWeaponType.블레이드) logger.debug(id);
					...
					// if (isWeapon(id)) shopId +=
					// ShopMaker.MapleWeaponType.getWeaponType(id).getShopIdAlter();
					// list.add(new Triple<Integer, Integer, Integer>(id,
					// shopId, price));
					// }
				} catch (Exception e) {
					logger.debug("{}", e);
	...
	private static enum EquipType {
		모자("cp", 1), 가발("hrcp", 1), 얼굴장식("af", 2), 눈장식("ay", 3), 귀고리("ae", 4), 신발("so", 5), 망토("sr", 6), 상의("ma", 7), 장갑("gv", 8), 한벌옷("mapn", 9), 하의("pn", 10), 반지("ri", 11), 방패("si", 12), 펜던트("pe",
				13), 허리띠("be", 13), 어깨장식("sh", 13), 훈장("me", 14), 무기("wp", 15), 보조무기("wpsi", 15), 쓰레기("gbg", 0),;
	...
	private enum MapleWeaponType {
		무기아님(0), 한손도끼(1), 두손도끼(2), 한손둔기(3), 두손둔기(4), 활(5), 아대(6), 석궁(7), 단검(8), 총(9), 너클(10), 폴암(11), 창(12), 스태프(13), 한손검(14), 두손검(15), 완드(16), 샤이닝로드(17), 소울슈터(18), 블레이드(19), 보조무기(20), 케인(21), 듀얼보우건(
				22),;
		...
		private static ShopMaker.MapleWeaponType getWeaponType(int itemId) {
			...
			ShopMaker.MapleWeaponType[] type = { 무기아님, 샤이닝로드, 소울슈터, 무기아님, 무기아님, 무기아님, 무기아님, 무기아님, 무기아님, 무기아님, 한손검, 한손도끼, 한손둔기, 단검, 블레이드, 보조무기, 케인, 완드, 스태프, 무기아님, 두손검, 두손도끼, 두손둔기, 창, 폴암, 활, 석궁, 아대, 너클,
					총, 무기아님, 무기아님, 듀얼보우건 };
