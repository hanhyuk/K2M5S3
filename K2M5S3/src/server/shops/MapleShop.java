			...
			if (c.getPlayer().getMeso() >= item.getPrice() * quantity) {
				if (InventoryManipulator.checkSpace(c, itemId, (short) (quantity * item.getQuantity()), "")) {
					if (GameConstants.isPet(itemId)) {
						InventoryManipulator.addById(c, itemId, (short) (quantity * item.getQuantity()), null,
								MaplePet.createPet(itemId, System.currentTimeMillis() + ((long) (item.getPeriod()) * ((long) 86400000))), 0,
								CurrentTime.getAllCurrentTime() + "에 " + c.getPlayer().getName() + "에서 호출된 " + getId() + " 상점에서 구매한 아이템.");
					} else {
						ItemInformation ii = ItemInformation.getInstance();
						if (GameConstants.isRechargable(itemId)) {
							quantity = ii.getSlotMax(c, item.getItemId());
							c.getPlayer().gainMeso(-(item.getPrice()), false);
							InventoryManipulator.addById(c, itemId, (short) (quantity * item.getQuantity()), null, null,
									item.getPeriod() <= 0 ? 0 : ((item.getPeriod() * 60L * 1000L) + System.currentTimeMillis()), CurrentTime.getAllCurrentTime() + "에 " + getId() + " 상점에서 구매한 아이템.");
						} else {
			...
			if (c.getPlayer().haveItem(item.getPrice(), item.getPriceQuantity() * quantity, false, true)) {
				if (InventoryManipulator.checkSpace(c, itemId, (short) (quantity * item.getQuantity()), "")) {
					if (GameConstants.isPet(itemId)) {
						InventoryManipulator.addById(c, itemId, (short) (quantity * item.getQuantity()), null,
								MaplePet.createPet(itemId, System.currentTimeMillis() + ((long) (item.getPeriod()) * ((long) 86400000))), 0,
								CurrentTime.getAllCurrentTime() + "에 " + getId() + " 상점에서 구매한 아이템.");
					} else {
						ItemInformation ii = ItemInformation.getInstance();
						if (GameConstants.isRechargable(itemId)) {
							quantity = ii.getSlotMax(c, item.getItemId());
		...
		try {
			...
			while (rs.next()) {
				if (GameConstants.isThrowingStar(rs.getInt("itemid")) || GameConstants.isBullet(rs.getInt("itemid"))) {
					MapleShopItem starItem = new MapleShopItem((short) 1, rs.getInt("itemid"), rs.getInt("price"), rs.getInt("pricequantity"), (byte) rs.getInt("Tab"), rs.getInt("quantity"),
							rs.getInt("period"));
					...
					ret.addItem(
							new MapleShopItem((short) 1000, rs.getInt("itemid"), rs.getInt("price"), rs.getInt("pricequantity"), (byte) rs.getInt("Tab"), rs.getInt("quantity"), rs.getInt("period")));
				}
			}
