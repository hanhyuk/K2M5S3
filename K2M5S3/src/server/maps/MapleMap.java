...
public class MapleMap {
	...
	public final Point calcPointMaple(final Point initial) {
		...
		if (fh == null) {
			logger.debug("[오류] 위치를 구하던 중 바닥의 풋홀드를 구하는데 실패했습니다.");
			return null;
		}
		int dropY = fh.getY1();
	...
	private void dropFromMonster(final MapleCharacter chr, final MapleMonster mob) {
		...
		final int cashModifier = (int) (mob.getMobExp() / 1000 + mob.getMobMaxHp() / 20000); // no
																								// rate
		for (final MonsterGlobalDropEntry de : globalEntry) {
			if (Randomizer.nextInt(999999) < de.chance) {
				if (de.itemId != 0) {
					...
					if ((de.itemId == 4001513 && !(mob.getStats().getLevel() >= 105 && mob.getStats().getLevel() <= 114))
							|| (de.itemId == 4001515 && !(mob.getStats().getLevel() >= 115 && mob.getStats().getLevel() <= 159))
							|| (de.itemId == 4001521 && !(mob.getStats().getLevel() >= 160 && mob.getStats().getLevel() <= 250))) {
						return;
					}
					spawnMobDrop(idrop, calcDropPos(pos, mob.getTruePosition()), mob, chr, droptype, de.questid);
					d++;
	...
	private final void killMonster(final MapleMonster monster) { // For mobs
																	// with
																	// removeAfter
	...
	public final void killMonster(final MapleMonster monster, final MapleCharacter chr, final boolean withDrops, final boolean second, final byte animation) {
		...
		if (monster.getBuffToGive() > -1) {
			...
			for (final MapleMapObject mmo : characters) {
				...
				if (c.isAlive()) {
					...
					case 8820001:
					case 8820101:
						c.getClient().getSession().write(MainPacketCreator.showOwnBuffEffect(buffid, 12)); // HT
																											// nine
																											// spirit
						broadcastMessage(c, MainPacketCreator.showBuffeffect(c.getId(), c.getLevel(), buffid, 0, 12), false); // HT
																																// nine
																																// spirit
						break;
					}
		...
		} else if (mobid == 8820008 || mobid == 8820108) { // wipe out statues
															// and respawn
		...
		if (monster.getId() == 8910001 || monster.getId() == 8900102 || monster.getId() == 8920103 || monster.getId() == 8930100) {
			...
			int i = monster.getId() == 8930100 ? 7 + Randomizer.nextInt(13)
					: monster.getId() == 8920103 ? 5 + Randomizer.nextInt(4) : monster.getId() == 8900102 ? 3 + Randomizer.nextInt(4) : 6 + Randomizer.nextInt(9);
		...
		if (!this.isEliteBossMap() && !monster.isEliteMonster() && !monster.isEliteBoss() && !monster.getStats().isBoss() && monster.getStats().getLevel() - 20 <= chr.getLevel()
			...
			tools.Timer.ShowTimer.getInstance().schedule(new Runnable() {
				...
				public final void run() {
					...
					/*
					 * 2432391 - 경험치(소) 2432392 - 경험치(대) 2432393 - 메소(소) 2432394
					 * - 메소(대) 2432395 - 포션 2432396 - 방어구 2432397 - 무기 2432398 -
					 * 희귀 아이템
					 */
				}
			}, 7000L);
		...
		if (this.EliteMobCommonCount == 200 && this.EliteMobCount >= 8) { // 보스
																			// 소환
			SetEliteMobCommonCount(0);
			SetEliteMobCount(0);
			...
			final String EffectMsgs[] = { "검은 기사 모카딘 : 위대한 분을 위하여 너를 처단하겠다.", "미친 마법사 카리아인 : 미천한 것들이 날뛰고 있구나. 크크크크...", "돌격형 CQ57 : 목표발견. 제거 행동을 시작한다.", "인간사냥꾼 줄라이 : 사냥감이 나타났군.",
				...
				public void run() {
					for (final MapleMapObject mo : getMapObjectsInRect(mist.getBox(), Collections.singletonList(MapleMapObjectType.MONSTER))) {
						if (mist.makeChanceResult()) {
							((MapleMonster) mo).applyStatus(mist.getOwner(), new MonsterStatusEffect(Collections.singletonMap(MonsterStatus.POISON, 1), mist.getSourceSkill(), null, false), true,
									duration, false);
						}
					}
				}
	...
	public final void spawnItemDrop(final MapleMapObject dropper, final MapleCharacter owner, final IItem item, Point pos, final boolean ffaDrop, final boolean playerDrop, boolean fly, boolean touch,
			int gradiant, int speed) {
	...
	private final void updateMapObjectVisibility(final MapleCharacter chr, final MapleMapObject mo) {
		...
		if (!chr.isMapObjectVisible(mo)) { // monster entered view range
			if (mo.getType() == MapleMapObjectType.MIST || mo.getType() == MapleMapObjectType.EXTRACTOR || mo.getType() == MapleMapObjectType.ANDROID || mo.getType() == MapleMapObjectType.SUMMON
			...
			} else if (mo.getType() == MapleMapObjectType.MONSTER) { // monster
																		// didn't
																		// leave
																		// view
																		// range,
	...
	public void setMapTimer(long time) {
		try {
			...
			logger.debug("[오류] DB로 보스 쿨타임을 저장하는데 실패했습니다. {}", e);
		}
		this.maptimer = time;
	}
	...
	public Collection<MapleCharacter> getNearestPvpChar(Point attacker, double maxRange, double maxHeight, boolean isLeft, Collection<MapleCharacter> chr) {
		...
		for (MapleCharacter a : characters) {
			if (chr.contains(a.getClient().getPlayer())) {
				...
				if (isLeft) {
					if (attacker.x < attackedPlayer.x && distanceX < maxRange && distanceX > 1 && attackedPlayer.y >= attacker.y - maxHeight && attackedPlayer.y <= attacker.y + maxHeight) {
						character.add(a);
					}
				} else {
					if (attacker.x > attackedPlayer.x && distanceX < maxRange && distanceX > 1 && attackedPlayer.y >= attacker.y - maxHeight && attackedPlayer.y <= attacker.y + maxHeight) {
					...
					for (MapleCharacter chr : getCharacters()) {
						...
						if (chr.isCatching) {
							chr.changeMap(chr.getClient().getChannelServer().getMapFactory().getMap(109090201),
									chr.getClient().getChannelServer().getMapFactory().getMap(109090201).getPortalSP().get(0));
						} else {
							chr.changeMap(chr.getClient().getChannelServer().getMapFactory().getMap(109090100),
									chr.getClient().getChannelServer().getMapFactory().getMap(109090100).getPortalSP().get(0));
						}
					}
					stopCatch();
				}
				...
				if (!CheckNPC(rs.getInt("lifeid"))) {
					{
						...
						if (npc != null) {
							...
							logger.debug("[오류] 엔피시 데이터를 만드는중 널 포인터 오류가 발생했습니다.");
						}
					}
				}
			}
		} catch (Exception e) {
			logger.debug("[오류] 엔피시를 DB로부터 불러오는데 오류가 발생했습니다. {}", e);
		}
	}
