...
public class MapleWorldMapProvider {
		...
		if (map == null) {
			synchronized (this) {
				...
				for (MapleData footRoot : mapData.getChildByPath("foothold")) {
					for (MapleData footCat : footRoot) {
						for (MapleData footHold : footCat) {
							fh = new MapleFoothold(new Point(MapleDataTool.getInt(footHold.getChildByPath("x1")), MapleDataTool.getInt(footHold.getChildByPath("y1"))),
					...
					try {
						...
						while (rs.next()) {
							...
							if (pnpc != null) {
								...
								logger.debug("[오류] 플레이어 엔피시 데이터를 만드는중 널 포인터 오류가 발생했습니다.");
							}
						}

					} catch (Exception e) {
						logger.debug("[오류] 플레이어 엔피시를 DB로부터 불러오는데 오류가 발생했습니다. {}", e);
					}
				...
				try {
					...
					logger.debug("[오류] 길드화분을 DB로부터 불러오는데 오류가 발생했습니다. {}", e);
				}
				...
				if (reactors) {
					try {
						...
						logger.debug("[오류] 광맥을 DB로부터 불러오는데 오류가 발생했습니다. {}", e);
					}
				}
				try {
	...
	public MapleMap CreateInstanceMap(int mapid, boolean respawns, boolean npcs, boolean reactors) {
		...
		for (MapleData footRoot : mapData.getChildByPath("foothold")) {
			for (MapleData footCat : footRoot) {
				for (MapleData footHold : footCat) {
					MapleFoothold fh = new MapleFoothold(new Point(MapleDataTool.getInt(footHold.getChildByPath("x1")), MapleDataTool.getInt(footHold.getChildByPath("y1"))),
							new Point(MapleDataTool.getInt(footHold.getChildByPath("x2")), MapleDataTool.getInt(footHold.getChildByPath("y2"))), Integer.parseInt(footHold.getName()));
					fh.setPrev((short) MapleDataTool.getInt(footHold.getChildByPath("prev")));
					fh.setNext((short) MapleDataTool.getInt(footHold.getChildByPath("next")));
