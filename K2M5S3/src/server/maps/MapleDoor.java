...
public class MapleDoor extends AbstractHinaMapObject {
    ...
    public final void sendSpawnData(final MapleClient client, boolean animated) {
        ...
        if (target.getId() == client.getPlayer().getMapId() || getOwner().getId() == client.getPlayer().getId() || (getOwner() != null && getOwner().getParty() != null && client.getPlayer().getParty() != null && getOwner().getParty().getId() == client.getPlayer().getParty().getId())) {
            if (!isTown) {
                client.getPlayer().getMap().broadcastMessage(client.getPlayer(), MainPacketCreator.spawnDoor(getOwner().getId(), target.getId() == client.getPlayer().getMapId() ? targetPosition : townPortal.getPosition(), animated), false); //spawnDoor always has same position.
            }
            if (getOwner() != null && getOwner().getParty() != null && client.getPlayer().getParty() != null && (getOwner().getId() == client.getPlayer().getId() || getOwner().getParty().getId() == client.getPlayer().getParty().getId())) {
                client.getPlayer().getMap().broadcastMessage(client.getPlayer(), MainPacketCreator.partyPortal(town.getId(), target.getId(), 2311002, target.getId() == client.getPlayer().getMapId() ? targetPosition : townPortal.getPosition(), animated), false);
            } else {
                client.getPlayer().getMap().broadcastMessage(client.getPlayer(), MainPacketCreator.spawnPortal(town.getId(), target.getId(), 2311002, target.getId() == client.getPlayer().getMapId() ? targetPosition : townPortal.getPosition()), false);
            }
        }
    }
    ...
    public final void sendDestroyData(final MapleClient client, boolean animated) {
        ...
        if (target.getId() == client.getPlayer().getMapId() || getOwner().getId() == client.getPlayer().getId() || (getOwner() != null && getOwner().getParty() != null && client.getPlayer().getParty() != null && getOwner().getParty().getId() == client.getPlayer().getParty().getId())) {
            client.getSession().write(MainPacketCreator.removeDoor(getOwner().getId(), animated));
            if (getOwner() != null && getOwner().getParty() != null && client.getPlayer().getParty() != null && (getOwner().getId() == client.getPlayer().getId() || getOwner().getParty().getId() == client.getPlayer().getParty().getId())) {
                client.getSession().write(MainPacketCreator.partyPortal(999999999, 999999999, 0, new Point(-1, -1), false));
            } else {
                client.getSession().write(MainPacketCreator.spawnPortal(999999999, 999999999, 0, null));
            }
        }
    }
    ...
    public final void warp(final MapleCharacter chr, final boolean toTown) {
        ...
        if (chr == getOwner() || getOwner().getParty() != null && getOwner().getParty().containsMembers(new MaplePartyCharacter(chr))) {
            if (!toTown) {
