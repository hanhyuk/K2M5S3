...
public class MakerItemFactory {
	...
	public static MakerItemFactory.MakerItemCreateEntry getItemCreateEntry(int toCreate) {
		...
		for (MapleData dt : makerData.getChildren()) // loop through every
														// category
			for (MapleData itemdata : dt.getChildren()) { // loop through all
															// the items in each
															// category
				...
				MakerItemFactory.MakerItemCreateEntry mice = new MakerItemFactory.MakerItemCreateEntry(cost, reqLevel, reqSkillLevel, amount);
				if (itemdata.getChildByPath("recipe") != null) // O_o
					for (MapleData recipedata : itemdata.getChildByPath("recipe").getChildren()) { // loop
																									// through
																									// all
																									// the
																									// needed
																									// items
						int itemid = MapleDataTool.getInt("item", recipedata, -1);// apperantly
																					// some
																					// of
																					// them
																					// are
