...
public class InventoryManipulator {
	...
	public static int addById(MapleClient c, int itemId, short quantity, String owner, MaplePet pet, long period, boolean justice, String log) {
		...
		if (!type.equals(MapleInventoryType.EQUIP)) {
			...
			if (!GameConstants.isThrowingStar(itemId) && !GameConstants.isBullet(itemId)) {
				...
				// Throwing Stars and Bullets - Add all into one slot regardless
				// of quantity.
	...
	public static void move(final MapleClient c, final MapleInventoryType type, final short src, final short dst) {
		...
		if (!type.equals(MapleInventoryType.EQUIP) && initialTarget != null && initialTarget.getItemId() == source.getItemId() && initialTarget.getExpiration() == source.getExpiration()
				&& !GameConstants.isThrowingStar(source.getItemId()) && !GameConstants.isBullet(source.getItemId()) && initialTarget.getOwner().equals(source.getOwner())) { // 아이템
																																												// 증발
																																												// 버그
																																												// 수정
	...
	public static void equip(final MapleClient c, final short src, short dst) {
		...
		case -1203: { // Top
			...
			if (bottom != null && GameConstants.isOverall(source.getItemId())) { // 하의를
																					// 낀채로
																					// 상의를
																					// 입을때
		...
		case -1204: { // Bottom
			...
			final IItem bottom = chr.getInventory(MapleInventoryType.EQUIPPED).getItem((short) bottomPos);
			if (top != null && GameConstants.isOverall(top.getItemId())) { // 한벌옷을
																			// 낀채로
																			// 하의를
																			// 입을때
				if (chr.getInventory(MapleInventoryType.EQUIP).isFull(bottom != null && GameConstants.isOverall(top.getItemId()) ? 1 : 0)) { // 낀
																																				// 상의가
																																				// 한벌옷이면
																																				// 여유공간이
																																				// 두칸은
		...
		target = (Equip) chr.getInventory(MapleInventoryType.EQUIPPED).getItem(dst); // Currently
																						// equipping
		...
		if (GameConstants.isWeapon(source.getItemId())) {
			...
			c.getPlayer().cancelEffectFromBuffStat(BuffStats.SOUL_WATK, -1);
		...
		if (stats.get("equipTradeBlock") == 1 || source.getItemId() / 10000 == 167 || source.getFire() >= 0) { // Block
																												// trade
																												// when
																												// equipped.
			...
			if (android != null) {
				...
				if (ItemFlag.ANDROID_ACTIVATED.check(flag)) {
					if (android.getAndroid() != null) {
						...
						logger.debug("[오류] {} 의 {} 유니크 아이디의 안드로이드에서 NULL 포인터가 발생했습니다.", chr.getName(), android.getUniqueId());
						return;
					}
				} else {
			...
			if (heart != null) {
				...
				if (ItemFlag.ANDROID_ACTIVATED.check(flag)) {
					if (source.getAndroid() != null) {
						...
						chr.dropMessage(1, "오류가 발생했습니다!");
						logger.debug("[오류] {} 의 {} 유니크 아이디의 안드로이드에서 NULL 포인터가 발생했습니다.", chr.getName(), source.getUniqueId());
						return;
					}
				} else {
	...
	public static void drop(final MapleClient c, MapleInventoryType type, final short src, final short quantity) {
		...
		final IItem source = c.getPlayer().getInventory(type).getItem(src);
		if (quantity < 0 || source == null || GameConstants.isPet(source.getItemId())
				|| quantity == 0 && !GameConstants.isThrowingStar(source.getItemId()) && !GameConstants.isBullet(source.getItemId())) {
			c.getSession().write(MainPacketCreator.resetActions());
			return;
		}
	...
	private final static IItem checkEnhanced(final IItem before, final MapleCharacter chr) {
		if (before instanceof Equip) {
			...
			if (eq.getState() == 0 && (eq.getUpgradeSlots() >= 1 || eq.getLevel() >= 1) && Randomizer.nextInt(100) > 92) { // 20%
																															// chance
																															// of
																															// pot?
				eq.setState((byte) 1);
				// chr.dropMessage(5, "You have obtained an item with hidden
				// Potential.");
			}
		}
		return before;
