...
public class ItemInformation {
	private static final Logger logger = LoggerFactory.getLogger(ItemInformation.class);

	private final static ItemInformation instance = new ItemInformation();
	public final MapleDataProvider etcData = MapleDataProviderFactory.getDataProvider("Etc.wz");
	public final MapleDataProvider itemData = MapleDataProviderFactory.getDataProvider("Item.wz");
	...
	public final int getSuccess(final int itemId, final MapleCharacter player, IItem equip) {
		...
		if (equip == null) {
			logger.debug("[오류] 주문서의 성공확률을 구하던 중, 장비 아이템 값에 널 값이 입력되었습니다. {}", itemId);
		...
		final MapleData item = getItemData(itemId);
		if (item == null) {
			logger.debug("[오류] 주문서의 성공확률을 구하던 중, 주문서 데이터 값에 널 값이 입력되었습니다. {}", itemId);
			player.message(5, "[오류] 현재 주문서의 성공확률을 구하는데 실패하였습니다.");
			player.gainItem(itemId, (short) 1, false, -1, "주문서 성공확률 얻기 실패로 얻은 주문서");
			player.ea();
			return 0;
	...
	public final BasicAndroid getAndroidBasicSettings(final int android) {
		...
		if (root == null) {
			logger.debug("[오류] 서버의 Etc.wz에서 안드로이드 기본 셋팅이 저장된 {} 파일을 발견하지 못했습니다.", name);
			return null;
		}
	...
	public final List<Pair<Integer, Integer>> getSummonMobs(final int itemId) {
		...
		for (final MapleData child : data.getChildren()) {
			mobPairs.add(new Pair(MapleDataTool.getIntConvert("id", child), MapleDataTool.getIntConvert("prob", child)));
		}
	...
	public final Equip randomizeStatsFire(final Equip equip, final boolean isFirst) {
		...
		equip.setMatk(getRandStatFire(equip.getMatk(), GameConstants.isWeapon(equip.getItemId()) ? 40 : 15, isFirst, "matk", equip.getUniqueId()));
		equip.setWatk(getRandStatFire(equip.getWatk(), GameConstants.isWeapon(equip.getItemId()) ? 40 : 15, isFirst, "watk", equip.getUniqueId()));
	...
	private final short getRandStatFire(final short originalValue, final int maxRange, final boolean isFirst, final String type, final int uniqueid) {
		// vary no more than ceil of 10% of stat
		Connection con = MYSQL.getConnection();
		PreparedStatement ps;
