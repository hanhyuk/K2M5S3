			...
			for (final MapleData iEntry : data.getChildren()) {
				...
				if (retitem == itemid) {
					if (!c.haveItem(retitem, 1, true, false)) {
						InventoryManipulator.addById(c.getClient(), retitem, (short) 1, null, null, 0,
								CurrentTime.getAllCurrentTime() + "에 " + c.getName() + "에서 호출된 " + itemid + " 아이템을 퀘스트 재료 다시받기로 얻음.");
					}
					return true;
				}
			}
		...
		case item:
			...
			for (MapleData iEntry : data.getChildren()) {
				...
				if (count < 0) { // remove items
					...
					InventoryManipulator.addById(c.getClient(), id,
							count/* , "", -1, 0 */, null, null, 0, CurrentTime.getAllCurrentTime() + "에 " + c.getName() + "에서 호출된 " + id + " 아이템에을 퀘스트 재료로 받은 아이템.");
					c.getClient().getSession().write(MainPacketCreator.getShowItemGain(id, count, true));
				}
			}
			break;
		...
		case item: {
			...
			for (MapleData iEntry : data.getChildren()) {
				...
				if (count < 0) { // remove items
					...
					// final int period =
					// MapleDataTool.getInt(iEntry.getChildByPath("period"), 0);
					InventoryManipulator.addById(c.getClient(), id, count,
							/* , -1, period * 60 * 1000 */ null, null, 0, CurrentTime.getAllCurrentTime() + "에 " + c.getName() + "에서 호출된 " + quest.id + "퀘스트에서 보상으로 얻은 아이템.");
					c.getClient().getSession().write(MainPacketCreator.getShowItemGain(id, count, true));
				}
			}
			break;
