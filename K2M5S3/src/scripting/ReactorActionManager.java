	...
	public void gainGuildPot() {
		...
		try {
			...
			if (reactor.getState() == 1) {
				...
				MapTimer.getInstance().schedule(new Runnable() {
					...
					public void run() {
						...
						// getPlayer().getGuild().guildBuff(skillids[Randomizer.rand(0,
						// 3)], 1);
					}
				}, 180000);
			...
			if (reactor.getGuildid() != getPlayer().getGuildId()) {
				// getPlayer().getClient().getSession().write(MaplePacketCreator.getNPCTalk(reactor.getReactorId(),
				// (byte) 0, "저는 #b" + reactor.getName() + "길드#k의
