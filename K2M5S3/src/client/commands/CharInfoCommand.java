...
public class CharInfoCommand implements Command {
	...
	public void execute(MapleClient c, String[] splittedLine) throws Exception, IllegalCommandSyntaxException {
		...
		builder.append(LogUtils.getLogMessage(other, ""));
		...
		builder.append(" || 힘 : ").append(other.getClient().getPlayer().getStat().getLocalStr()).append(" (")
				...
				.append(other.getClient().getPlayer().getStat().getLocalStr() - other.getClient().getPlayer().getStat().getStr()).append(")");
		builder.append(" || 덱스 : ").append(other.getClient().getPlayer().getStat().getLocalDex()).append(" (")
				...
				.append(other.getClient().getPlayer().getStat().getLocalDex() - other.getClient().getPlayer().getStat().getDex()).append(")");
		builder.append(" || 인트 : ").append(other.getClient().getPlayer().getStat().getLocalInt()).append(" (")
				...
				.append(other.getClient().getPlayer().getStat().getLocalInt() - other.getClient().getPlayer().getStat().getInt()).append(")");
		builder.append(" || 럭 : ").append(other.getClient().getPlayer().getStat().getLocalLuk()).append(" (")
				...
				.append(other.getClient().getPlayer().getStat().getLocalLuk() - other.getClient().getPlayer().getStat().getLuk()).append(")");
	...
	public CommandDefinition[] getDefinition() {
		return new CommandDefinition[] { new CommandDefinition("캐릭정보", "<캐릭터이름>", "입력한 이름의 캐릭터의 정보를 출력합니다.", 3) };
	}
}
