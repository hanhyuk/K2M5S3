...
public class TestCommands implements Command {
	...
	public void execute(final MapleClient c, final String[] splitted) throws Exception, IllegalCommandSyntaxException {
		if (splitted[0].equals("!시계")) {
			c.getPlayer().getMap().broadcastMessage(MainPacketCreator.getClock(CommandProcessor.getInstance().getOptionalIntArg(splitted, 1, 60)));
	...
	public CommandDefinition[] getDefinition() {
		return new CommandDefinition[] { new CommandDefinition("시계", "<시간>", "현재 맵에 시간을 띄웁니다.", 2), new CommandDefinition("패킷", "<헥스데이터>", "입력한 16진수 데이터의 패킷을 현재 클라이언트에 보냅니다.", 6), };
	}
}
