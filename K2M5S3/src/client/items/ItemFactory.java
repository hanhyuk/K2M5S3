...
public class ItemFactory {
    ...
    public static void saveItems(MapleCharacter hp, ItemFactory.InventoryType invtype, MapleStorage store, Pair<Integer, List<IItem>> pack, MapleCashInventory cash, List<AuctionItemPackage> aitems) {
        try {
            Connection con = MYSQL.getConnection();
            PreparedStatement ps = null, pse = null;
            if (invtype == ItemFactory.InventoryType.AUCTION) {
                ps = con.prepareStatement("INSERT INTO auctionitems (characterid, type, accountid, packageId, itemid, inventorytype, position, quantity, owner, GM_Log, uniqueid, expiredate, flag, giftFrom, isCash, isPet, isAndroid, bid, meso, expired, bargain, ownername, buyer, buytime, starttime, `status`, inventoryitemid) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)", MYSQL.RETURN_GENERATED_KEYS);
            ...
            for (Pair<IItem, Byte> item : items) {
                ...
                if (invtype == ItemFactory.InventoryType.AUCTION) {
                    ...
                    ps.setLong(13, aitem.getBid());
                    ps.setLong(14, aitem.getMesos());
                    ps.setLong(15, aitem.getExpiredTime());
                    ps.setBoolean(16, aitem.isBargain());
                    ps.setString(17, aitem.getOwnerName());
                    ps.setInt(18, aitem.getBuyer());
                    ps.setLong(19, aitem.getBuyTime());
                    ps.setLong(20, aitem.getStartTime());
                    ps.setInt(21, aitem.getType(false, true));
                    ps.setLong(22, item.getLeft().getInventoryId());
                }
                ps.executeUpdate();
                ...
                if (!rs.next()) {
                    ...
                    if (item.getRight().equals(MapleInventoryType.EQUIP.getType()) || item.getRight().equals(MapleInventoryType.EQUIPPED.getType())) {
                        ...
                        if (store == null) {
                            deleteEquipmentItem(type); //창고는 기존 데이터 사용.
                        }
                        pse.setInt(1, type);
    ...
    public static void deleteEquipmentItem(int inventoryid) {
        try {
            Connection con = MYSQL.getConnection();
            /* 인벤토리 장비 삭제 시작 */
