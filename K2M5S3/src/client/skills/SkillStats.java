...
public class SkillStats {
    ...
    public final void setStats(final String name, final String fomular, boolean rn) {
        ...
        if (fomular != null) {
            ...
            if (!rn) {
                if ((fomular.length() > 4) && (fomular.substring(fomular.length() - 4, fomular.length()).equals("\\r\\n"))) { //Do not Skill Loading Fixed.
                    fomular_r = fomular_r.replace(fomular.substring(0, fomular.length()), fomular.substring(0, fomular.length() - 4));
                }
                if (fomular.substring(0, 1).equals("-")) { //-30+3*x
                    fomular_r = fomular_r.replace(fomular.substring(0, fomular.length()), "n" + fomular.substring(1, fomular.length())); //n30+3*x
                }
