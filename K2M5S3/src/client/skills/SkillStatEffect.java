...
public class SkillStatEffect {
	...
	private static SkillStatEffect loadFromData(final MapleData source, final int sourceid, final boolean skill,
		...
		if (ret.overTime && ret.getSummonMovementType() == null && ret.absstats) {
			...
			if (sourceid != 21101005 || sourceid != 22131001) { // 매직 실드, 드레인은
																// 패시브효과임
		...
		if (hpchange != 0) {
			...
			if (sourceid == 2321007) {
				applyfrom.getClient().getSession()
						.write(MainPacketCreator.showOwnBuffEffect(sourceid, 1, applyfrom.getLevel(), 1)); // 일부
																											// 모션
																											// 안보이던
																											// 현상
																											// 수정.
				...
				if (affected != applyfrom && (isGmBuff() || applyfrom.getParty().equals(affected.getParty()))) {
					if ((isResurrection() && !affected.isAlive()) || (!isResurrection() && affected.isAlive())) {
						...
						affected.getClient().getSession()
								.write(MainPacketCreator.showOwnBuffEffect(sourceid, 3, affected.getLevel(), level)); // 일부
																														// 모션
																														// 안보이던
																														// 현상
																														// 수정.
	...
	private final void applyBuffEffect(final MapleCharacter applyfrom, final MapleCharacter applyto,
		...
		if (showEffect) {
			applyto.getClient().getSession()
					.write(MainPacketCreator.showOwnBuffEffect(sourceid, 1, applyto.getLevel(), level)); // 스킬
																											// 모션
																											// 안보이던
																											// 현상
																											// 수정.
			...
			case 2300000:
				...
				if (!mob.getStats().isBoss()) {
					...
					if (absorbMp > 0) {
						...
						applyto.getClient().getSession()
								.write(MainPacketCreator.showOwnBuffEffect(sourceid, 1, applyto.getLevel(), level)); // 일부
																														// 모션
																														// 안보이던
																														// 현상
																														// 수정.
