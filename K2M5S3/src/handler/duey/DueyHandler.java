...
public class DueyHandler {
    ...
    public static void DueyHandler(ReadingMaple rm, MapleCharacter chr) {
        ...
        switch(value) {
            ...
            default:
            {
                ...
                InventoryManipulator.removeFromSlot(chr.getClient(), MapleInventoryType.getByType((byte)type), (byte)pos, (short)quantity, false);
    ...
    public static Packet removeItem(int invid, boolean del) {
    ...
    public static Packet DueyMessage(int value) {
        WritingPacket packet = new WritingPacket();
        packet.writeShort(SendPacketOpcode.DUEY_HANDLER.getValue());
        packet.write(value);
        if(value == 28) {
    ...
    public static Packet sendDuey(int type, String name) {
        WritingPacket packet = new WritingPacket();
        packet.writeShort(SendPacketOpcode.DUEY_HANDLER.getValue());
        packet.write(type);
    ...
    public static void delDueyItem(int invid) {
        try {
            ...
            ps = MYSQL.getConnection().prepareStatement("SELECT * FROM dueyitems WHERE inventoryitemid = ?");
            ps.setInt(1, invid);
            ResultSet rs = ps.executeQuery();
            while (rs.next()) {
                ps = MYSQL.getConnection().prepareStatement("DELETE FROM dueyitems WHERE inventoryitemid = ?");
                ps.setInt(1, invid);
                ps.execute();
            }
    ...
    public static void gainDueyItem(MapleCharacter chr, int invid) {
        try {
            PreparedStatement dueydb = MYSQL.getConnection().prepareStatement("SELECT * FROM dueyitems WHERE inventoryitemid = ?");
    ...
    public static void DueyItemInfo(WritingPacket packet, String name) {
        ...
        try {
            PreparedStatement dueydb = MYSQL.getConnection().prepareStatement("SELECT * FROM dueyitems WHERE name = ?");
    ...
    public static void saveDueyItems(int type, int pos, int quantity, int mesos, String name,MapleCharacter chr) {
        try {                                                                                              
            PreparedStatement pse = MYSQL.getConnection().prepareStatement("INSERT INTO dueyitems VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
