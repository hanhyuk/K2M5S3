...
public class PlayerHandler {
    ...
    public static void TakeDamage(ReadingMaple rh, MapleClient c, MapleCharacter chr) throws InterruptedException {
        ...
        if (type != -2 && type != -3 && type != -4) { // Not map damage
            ...
            if (chr.getSkillLevel(36110004) > 0) {
                ...
                chr.getMap().broadcastMessage(chr, MainPacketCreator.attack(RecvPacketOpcode.CLOSE_RANGE_ATTACK, chr, chr.getId(), attack.tbyte, attack.skill, chr.getSkillLevel(36110004), attack.display, attack.speed, attack.allDamage, attack.value, attack.position, (byte) 0, 0, chr.getLevel(), 0), chr.getPosition());
            }
            ...
            if (damage != -1) {
                if ((chr.getJob() >= 411 && chr.getJob() <= 412) || (chr.getJob() >= 421 && chr.getJob() <= 422)) {
                    ...
                    if (summon != null) {
                        ...
                        chr.getMap().broadcastMessage(null, MainPacketCreator.summonAttack(chr.getId(), summon.getObjectId(), (byte) 0x78, (byte) 0x11, allDamage, chr.getLevel(), true, 0), summon.getPosition());
            ...
            if (attacker != null && chr.getBuffedValue(BuffStats.BEHOLDER, 1301013) != null && chr.getSkillLevel(1320011) > 0 && chr.getSummons().get(1301013) != null) {
                ...
                if (!chr.skillisCooling(1320011) && revenge.makeChanceResult()) {
                    ...
                    if (hs != null) {
                        ...
                        chr.getMap().broadcastMessage(MainPacketCreator.summonAttack(hs.getOwner().getId(), hs.getObjectId(), (byte) 0x78, (byte) 0x11, allDamage, chr.getLevel(), true, 0), hs.getPosition());
                        chr.checkMonsterAggro(attacker);
                ...
                if (skill.isChargeSkill()) {
                    if (sourceid == 3121004 ? skill.getEffect(20).getCooldown() > 0 : sourceid == 3121009 ? skill.getEffect(20).getCooldown() > 0 : skill.getEffect(chr.getSkillLevel(skill)).getCooldown() > 0) {
                        if (!chr.isEquilibrium() && !GameConstants.isDarkSkills(skill.getId())) {
                            ...
                            chr.addCooldown(skill.getId(), System.currentTimeMillis(), effect.getCooldown(), BuffTimer.getInstance().schedule(new CancelCooldownAction(chr, skill.getId()), effect.getCooldown()));
                        }
                    }
    ...
    public static void SpecialSkill(ReadingMaple rh, final MapleClient c, final MapleCharacter chr) {
        ...
        switch (skillid) {
            ...
            case 12120014: { // 스피릿 오브 플레임
                ...
                chr.getClient().getSession().write(MainPacketCreator.giveBuff(skillid, SkillFactory.getSkill(skillid).getEffect(chr.getSkillLevel(skillid)).getDuration(), stat2, SkillFactory.getSkill(skillid).getEffect(chr.getSkillLevel(skillid)), null, SkillFactory.getSkill(skillid).getAnimationTime()));
                break;
            }
            case 1311013: { //비홀더 도미넌트
    ...
    public static void closeRangeAttack(ReadingMaple rh, final MapleClient c, final MapleCharacter chr, boolean touchAttack) {
        ...
        if (attack.skill != 0) {    
            ...
            if ((skill.getId() == 31011000 || skill.getId() == 31201000 || skill.getId() == 31211000 || skill.getId() == 31221000) && attack.targets > 0) {
                ...
                final List<Triple<BuffStats, Integer, Boolean>> stat = Collections.singletonList(new Triple<BuffStats, Integer, Boolean>(BuffStats.ENHANCED_MAXMP, Integer.valueOf(c.getPlayer().exeedAttackCount > 4 ? 4 : c.getPlayer().exeedAttackCount), false));
                    ...
                    if (attack.skill == 1321013) {
                        ...
                        BuffTimer.getInstance().schedule(new Runnable() {
                            ...
                            public void run() {
                                ...
                                chr.addCooldown(65121052, System.currentTimeMillis(), sneffect.getCooldown(), BuffTimer.getInstance().schedule(new CancelCooldownAction(chr, 65121052), sneffect.getCooldown()));
                            }
                        }, 12000);
                        ...
                        BuffTimer.getInstance().schedule(new Runnable() {
                            ...
                            public void run() {
                                ...
                                chr.addCooldown(5121013, System.currentTimeMillis(), sneffect.getCooldown(), BuffTimer.getInstance().schedule(new CancelCooldownAction(chr, 5121013), sneffect.getCooldown()));
                            }
                        }, 12000);
                        ...
                        BuffTimer.getInstance().schedule(new Runnable() {
                            ...
                            public void run() {
                                ...
                                chr.addCooldown(4221052, System.currentTimeMillis(), sneffect.getCooldown(), BuffTimer.getInstance().schedule(new CancelCooldownAction(chr, 4221052), sneffect.getCooldown()));
                            }
                        }, 12000);
                    } else {
                        if (!chr.isEquilibrium() && !GameConstants.isDarkSkills(attack.skill) && skill.getId() != 31221001) {
                            ...
                            chr.addCooldown(attack.skill, System.currentTimeMillis(), SkillFactory.getSkill(attack.skill).getEffect(skillLevel).getCooldown(), BuffTimer.getInstance().schedule(new CancelCooldownAction(chr, attack.skill), SkillFactory.getSkill(attack.skill).getEffect(skillLevel).getCooldown()));
                        }
                    }
                }
        ...
        if (attack.skill == 21101003) {
            ...
            chr.getMap().broadcastMessage(chr, MainPacketCreator.attack(RecvPacketOpcode.CLOSE_RANGE_ATTACK, chr, chr.getId(), attack.tbyte, attack.skill, skillLevel, attack.display, attack.speed, attack.allDamage, attack.value, attack.position, (byte) 0, 0, chr.getLevel(), 0), chr.getPosition());
        }
        DamageParse.applyAttack(attack, skill, c.getPlayer(), attackCount, ((GameConstants.isKaiser(chr.getJob()) || touchAttack) ? null : effect), mirror ? AttackType.NON_RANGED_WITH_MIRROR : AttackType.NON_RANGED);        
    }
    ...
    public static void rangedAttack(ReadingMaple rh, MapleClient c, MapleCharacter chr) {
        ...
        if (chr.getBuffedValue(BuffStats.SOUL_WATK) == null && attack.skill != 4111004
        ...
        chr.getMap().broadcastMessage(chr, MainPacketCreator.attack(RecvPacketOpcode.RANGED_ATTACK, chr, chr.getId(), attack.tbyte, attack.skill, skillLevel, attack.display, attack.speed, attack.allDamage, attack.value, attack.position, (byte) 0, 0, chr.getLevel(), visProjectile), chr.getPosition());
    ...
    public static void MagicDamage(ReadingMaple rh, MapleClient c, MapleCharacter chr, boolean isFlame) {
        ...
        chr.getMap().broadcastMessage(chr, MainPacketCreator.attack(RecvPacketOpcode.MAGIC_ATTACK, chr, chr.getId(), attack.tbyte, attack.skill, skillLevel, attack.display, attack.speed, attack.allDamage, attack.value, attack.position, (byte) 0, attack.charge, chr.getLevel(), 0), chr.getPosition());
            ...
            case 1:
                ...
                for (Pair<Integer, Integer> p : recipe.getReqItems()) {
                    ...
                    if (item != null) {
                        ...
                        c.getPlayer().getMap().broadcastMessage(MainPacketCreator.showSpecialEffect(0x11)); //1.2.251+, (+1)
    ...
    public static void mistSkill(ReadingMaple rh, MapleCharacter chr) {
        ...
        for (int i = 0; i < (size * 2); i++) {
            ...
            chr.getMap().spawnMist(new MapleMist(effect.calculateBoundingBox(pos, chr.isFacingLeft()), chr, effect), (skillId == 2100010) ? (duration * 20) : (size * 2000), false, false, false, false, false);
        }
    }
