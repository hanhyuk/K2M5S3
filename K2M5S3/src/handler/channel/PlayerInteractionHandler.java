...
public class PlayerInteractionHandler {
	...
	public static final byte SET_ITEMS1 = 0, SET_ITEMS2 = 1, SET_ITEMS3 = 2, SET_ITEMS4 = 3, SET_MESO1 = 4,
			...
			BUY_ITEM_STORE3 = 38, MAINTANCE_OFF = 39, REMOVE_ITEM = 47, // +10
																		// 시작
			...
			ADMIN_STORE_NAMECHANGE = 59, START_ROCK_PAPER_SCISSORS1 = 96, START_ROCK_PAPER_SCISSORS2 = 97,
			START_ROCK_PAPER_SCISSORS3 = 98, INVITE_ROCK_PAPER_SCISSORS = 112, FINISH_ROCK_PAPER_SCISSORS = 113;
	...
	public static final void PlayerInteraction(final ReadingMaple rh, MapleClient c, final MapleCharacter chr) {
		...
		case CREATE: {
			...
			if (createType == 7 || createType == 4) { // trade
				chr.isTradeType = (createType == 7);
				MapleUserTrade.startTrade(chr, chr.isTradeType);
			} else if (createType == IMapleCharacterShop.HIRED_MERCHANT
					|| createType == IMapleCharacterShop.PLAYER_SHOP) { // shop
		...
		case INVITE_TRADE: {
			...
			MapleUserTrade.inviteTrade(chr, ochr, ochr.isTradeType);
			break;
		}
		...
		case BUY_ITEM_HIREDMERCHANT: { // Buy and Merchant buy
			...
			if (/* check > 32767 || */check <= 0 || check2 > 2147483647 || check2 <= 0 || check3 > 32767
...
//				}
			...
			int cid = rh.readInt();
			c.getSession().write(PlayerShopPacket.MaintanceCheck(mode, correct, cid));
			break;
		}
		default: {
