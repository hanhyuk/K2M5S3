	    ...
	    if (!chr.isGM() && text.length() >= 80) {
           ...
           if(text.charAt(0) == '~') {
               ...
               if (ServerConstants.chatlimit >= 500) {
                ...
                if (chr.getRC() > 0) {
                    WorldBroadcasting.broadcastMessage(MainPacketCreator.getGMText(21,"<후원 유저> " + c.getPlayer().getName() + " [Ch:" + (c.getChannel() == 0 ? 1 : c.getChannel() == 1 ? "20세이상" : c.getChannel()) + "]  : " + text.substring(1).toString()));
                } else {
