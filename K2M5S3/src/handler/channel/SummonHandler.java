...
public class SummonHandler {
	...
	private static final Logger logger = LoggerFactory.getLogger(SummonHandler.class);
	...
	public static final void SummonAttack(final ReadingMaple rh, final MapleClient c, final MapleCharacter chr) {
		...
		map.broadcastMessage(chr,
				MainPacketCreator.summonAttack(summon.getOwner().getId(), summon.getSkill(), ret.animation, ret.tbyte, ret.allDamage, chr.getLevel(), summon.getSkill() == 345174, value),
				summon.getPosition());
		...
		for (final AttackPair oned : ret.allDamage) {
			...
			if (toDamage > 0) {
				if (summonSkill.getId() == 23111008) { // 엘리멘탈 나이트
					if (summonEffect.makeChanceResult()) {
						mob.applyStatus(chr, new MonsterStatusEffect(Collections.singletonMap(MonsterStatus.FREEZE, 1), summonSkill, null, false), false,
								summonEffect.getSkillStats().getStats("subTime"), false);
					}
				}
				if (summonSkill.getId() == 23111009) { // 엘리멘탈 나이트
					if (summonEffect.makeChanceResult()) {
						mob.applyStatus(chr, new MonsterStatusEffect(Collections.singletonMap(MonsterStatus.VENOMOUS_WEAPON, summonEffect.getSkillStats().getStats("dot")), summonSkill, null, false),
								true, summonEffect.getSkillStats().getStats("dotTime"), false);
					}
				}
			}
	...
	public static final void removeSummon(final ReadingMaple rh, final MapleClient ha) {
		...
		if ((MapleSummon) ha.getPlayer().getMap().getMapObject(oid) != null) {
			...
			logger.debug("[오류] 현재 소환된 소환수들중 해당하는 오브젝트를 발견하지 못했습니다. : {}", oid);
		}
	}
	...
	public static final void subThingSummon(final ReadingMaple rh, final MapleClient ha, final MapleCharacter chr) {
		...
		if (skillid == 35121009) {
			...
			for (int i = 0; i < 3; i++) {
				final MapleSummon tosummon = new MapleSummon(ha.getPlayer(), SkillFactory.getSkill(35121011).getEffect(hs.getSkillLevel()), new Point(hs.getPosition().x, hs.getPosition().y - 5),
