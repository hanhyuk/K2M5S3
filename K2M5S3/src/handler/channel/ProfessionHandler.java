...
public class ProfessionHandler {
	private static final Logger logger = LoggerFactory.getLogger(ProfessionHandler.class);
	...
	public static void startGathering(ReadingMaple rh, MapleClient c) {
		...
		if (c.getPlayer().getMap().getMapObject(oid) != null) {
			...
			c.getPlayer().send(MainPacketCreator.startGathering(oid, 0x0B));
	...
	public static void professionMake(ReadingMaple rh, MapleClient c) {
		...
		if (playerSkillLevel < data.getReqSkillLevel()) {
			logger.debug("[오류] 전문기술 제작 필요 레벨이 부족합니다.");
			return;
		}
		...
		if (skillid == 92049000) { // disassembling
			...
			/*
			 * for (Pair<Integer, Integer> p : recipes) { if
			 * (!chr.haveItem(p.getLeft(), p.getRight(), false, true)) { return;
			 * } }
			 */
			fatigue = chr.getProfession().getFatigue();
			if (playerSkillExp < data.getReqSkillProficiency()) {
				logger.debug("[오류] 전문기술 제작 필요 숙련도가 부족합니다.");
				return;
			}
			if (fatigue + data.getIncFatigability() > 200) {
				logger.debug("[오류] 전문기술 제작 필요 피로도가 부족합니다.");
				return;
			}
			...
			chr.getMap().broadcastMessage(chr,
					MainPacketCreator.showProfessionMakeResult(chr.getId(), skillid, cg.grade, maked == null ? 0 : maked.getItemId(), maked == null ? 0 : maked.getQuantity(), exp), true);
		}
		c.getPlayer().setKeyValue("LastRecoveredFatigue", System.currentTimeMillis() + "");
	}

	public enum CraftGrade {
		FAIL(15, 0), SOSO(19, 30), GOOD(20, 40), COOL(21, 50);
	...
	public static final void spawnExtractor(ReadingMaple rh, MapleClient c) {
		...
		chr.setExtractor(new MapleExtractor(chr, itemId, fee, chr.getMap().getFootholds().findMaple(chr.getPosition()).getId())); // no
																																	// clue
																																	// about
																																	// time
																																	// left
	...
	public static final void useRecipe(ReadingMaple rh, MapleClient c) {
		...
		if (chr.getInventory(MapleInventoryType.USE).getItem(slot).getItemId() != itemid) {
			chr.ea();
			logger.debug("[오류] 슬롯에 있는 아이템과 사용한 아이템이 다릅니다.");
			return;
		}
		ItemInformation ii = ItemInformation.getInstance();
