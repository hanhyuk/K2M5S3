...
public class AllianceHandler {
	...
	public static final void AllianceOperatopn(final ReadingMaple rh, final MapleClient c, boolean denied) {
		...
		byte op = rh.readByte();
		if (c.getPlayer().getGuildRank() != 1 && op != 1) { // only updating
															// doesn't need
															// guild leader
			return;
		}
		...
		} else if (leaderid > 0 || gs.getAllianceId() > 0) { // infact, if they
																// have
																// allianceid
																// it's
																// suspicious
		...
		case 3: // invite
			...
			break;
		case 4: // accept invite... guildid that invited(int, a/b check) ->
				// guildname that was invited? but we dont care about that
		...
		case 10: // notice update
			...
			break;
		default:
			logger.debug("Unhandled GuildAlliance op: {}, \n {}", op, rh.toString());
			break;
		}
		// c.getSession().write(MainPacketCreator.enableActions());
	}

	public static final void DenyInvite(MapleClient c, final MapleGuild gs) { // playername
																				// that
																				// invited
																				// ->
																				// guildname
