...
public class DamageParse {
    ...
    public static void doHideAndSeek(MapleCharacter player, AttackInfo attack, boolean catched) {
        for (MapleCharacter attackedPlayers : player.getMap().getNearestPvpChar(player.getPosition(), 91, 40, (attack.display >= 0), Collections.unmodifiableCollection(player.getMap().getCharacters()))) {
    ...
    public static void applyAttack(AttackInfo attack, ISkill theSkill, final MapleCharacter player, int attackCount, SkillStatEffect effect, AttackType attack_type) {
        ...
        for (final AttackPair oned : attack.allDamage) {
            ...
            if (monster != null) {
                ...
                if (GameConstants.isKaiser(player.getJob())) {
                    ...
                    if (player.getBuffedValue(BuffStats.LUMINOUS_GAUGE) == null || player.getBuffedValue(BuffStats.LUMINOUS_GAUGE) == -1) {
                    ...
                    switch (attack.skill) {
                        ...
                        case 101120204:
                            if (player.getSkillLevel(100000276) < 10) {
                                player.getClient().getSession().write(MainPacketCreator.showOwnBuffEffect(101120207, 3)); 
                ...
                if (totDamageToOneMonster > 0) {
                    ...
                    if (player.getSkillLevel(30010111) > 0) { //데쓰 커스
                        ...
                        if (effs.makeChanceResult()) {
                            if (!monster.getStats().isBoss() && player.getMapId() != 900000000 && player.getMapId() != 109040004) { //보스가 아니어야함.
                                ...
                                player.addHP((long) (player.getStat().getCurrentMaxHp() * (effs.getX() / 100.0D))); //체력 회복
                            }
                        }
                    }
                    ...
                    if (player.getJob() >= 310 && player.getJob() <= 312) {
                        if (player.quiver) {
                            if (player.quivermode == 1) {
                                ...
                                } else if (player.quivercount[0] == 1) {//흡혈이 동났어
                                ...
                                if (player.quivercount[1] > 1) {//독
                                    ...
                                    for (int i = 0; i < attack.allDamage.size(); ++i) { //마리수
                                        MapleMonster Target = player.getMap().getMonsterByOid(attack.allDamage.get(i).objectid);
                                        if (Randomizer.nextInt(20) < 100) {
                                            Target.applyStatus(player, new MonsterStatusEffect(Collections.singletonMap(MonsterStatus.POISON, (int) Randomizer.rand(100, 500)), SkillFactory.getSkill(90001003), null, false), false, (long) 10 * 1000, false);
                                        }
                                    }
                                } else if (player.quivercount[1] == 1) {//독이 동났어
                                ...
                                if (player.quivercount[2] > 1) {//마법 화살
                                    ...
                                    if (effect.makeChanceResult()) {
                                        player.getClient().getSession().write(MainPacketCreator.giveMagicArrow(player));
                                    }
                                } else if (player.quivercount[2] == 1) {//마법 화살이 동났어
                                    player.quivercount[2] += (9 + (player.getSkillLevel(3121016) * 3));
                                    player.quivermode = 1;
                                }
                            }
                            player.getClient().getSession().write(MainPacketCreator.QuiverKartrige((player.quivercount[0] * 10000) + (player.quivercount[1] * 100) + (player.quivercount[2] * 1), player.quivermode));
                        }
                    }
                    if (player.getSkillLevel(5100015) > 0) {
                        if (attack.skill > 0) {
                            final int count = player.acaneAim + 1;
                            if (player.getBuffedValue(BuffStats.CHARGING_ENERGY, 5121054) == null) {
                                if ((!player.EnergyCharge) && (count * 350 < 9800)) {
                                    player.send(MainPacketCreator.giveEnergyCharge(count * 350, 9800, 0, 0, false, false));
                                    player.getMap().broadcastMessage(player, (MainPacketCreator.giveForeignEnergyCharge(player.getId(), count * 350)), false);
                                    player.acaneAim++;
                                } else if ((!player.EnergyCharge) && (count * 350 == 9800)) {
                                    player.EnergyCharge = true;
                                    player.send(MainPacketCreator.giveEnergyCharge(0, 0, 5120018, 10000, true, false));
                                    player.getMap().broadcastMessage(player, (MainPacketCreator.giveForeignEnergyCharge(player.getId(), 10000)), false);
                                } else if ((player.EnergyCharge) && (count * 350 >= 700) && (count * 350 <= 9800)) { 
                                    player.send(MainPacketCreator.giveEnergyCharge(0, 0, 5120018, count * 350, false, true));
                                    player.acaneAim--;
                                } else {
                    ...
                    if (GameConstants.isNightWalker(player.getJob()) && player.getSkillLevel(14000027) > 0 && attack.targets > 0 && Randomizer.isSuccess(30)) {
                        int BatLimit = player.getSkillLevel(14000027) > 0 ? player.getSkillLevel(14100027) > 0 ? player.getSkillLevel(14110029) > 0 ? player.getSkillLevel(14120008) > 0 ? 5 : 4 : 3 : 2 : 0;
                        if (currentBat < BatLimit) {
                            if (currentBat > 0) {
                                ...
                                if (monstersInRange.size() > 0) {
                                    ...
                                    player.getMap().broadcastMessage(MainPacketCreator.giveShadowBat(player.getId(), monstersInRange.get(random).getObjectId(), player.getTruePosition()));
                                    monstersInRange.remove(random);
                                }
                            }
                            if (currentBat < 3) {
                                SkillStatEffect batskill = SkillFactory.getSkill(14000027).getEffect(player.getSkillLevel(14000027));
                                MapleSummon summon = new MapleSummon(player, 14000027, player.getPosition(), batskill.getSummonMovementType());
                                player.getMap().spawnSummon(summon, true, batskill.getDuration());
                                player.getSummons().put(14000027, summon);
                            }
                        }
                    }
                    if (player.getJob() >= 1500 && player.getJob() <= 1512) {
                        SkillStatEffect dkeffect = player.getBuffedSkillEffect(BuffStats.ELEMENT_LIGHTNING, 15001022);
                        if (dkeffect != null) {
                                        ...
                                        if (Randomizer.nextInt(100) <= pot.prop) {
                                            switch (i) {
                                                ...
                                                case 20206:
                                                    ...
                                                    break;
                                                case 10221:
                                                    monster.applyStatus(player, new MonsterStatusEffect(Collections.singletonMap(MonsterStatus.POISON, (int) pot.level), SkillFactory.getSkill(90001003), null, false), false, (long) 5 * 1000, false);
                                                    break;
                                                case 10226:
                                                    monster.applyStatus(player, new MonsterStatusEffect(Collections.singletonMap(MonsterStatus.STUN, (int) pot.level), SkillFactory.getSkill(90001001), null, false), false, (long) 5 * 1000, false);
                                                    break;
                                                case 10231:
                                                    monster.applyStatus(player, new MonsterStatusEffect(Collections.singletonMap(MonsterStatus.SPEED, (int) pot.level), SkillFactory.getSkill(90001002), null, false), false, (long) 5 * 1000, false);
                                                    break;
                                                case 10236:
                                                    monster.applyStatus(player, new MonsterStatusEffect(Collections.singletonMap(MonsterStatus.DARKNESS, (int) pot.level), SkillFactory.getSkill(90001004), null, false), false, (long) 5 * 1000, false);
                                                    break;
                                                case 10241:
                                                    monster.applyStatus(player, new MonsterStatusEffect(Collections.singletonMap(MonsterStatus.FREEZE, (int) pot.level), SkillFactory.getSkill(90001006), null, false), false, (long) 5 * 1000, false);
                                                    break;
                                                case 10246:
                                                    monster.applyStatus(player, new MonsterStatusEffect(Collections.singletonMap(MonsterStatus.SEAL, (int) pot.level), SkillFactory.getSkill(90001005), null, false), false, (long) 5 * 1000, false);
                                                    break;
                                            }
                                        }
                                    }
                    ...
                    if (player.getBuffedValue(BuffStats.SPIRIT_LINK) != null && totDamageToOneMonster <= 999999 && totDamageToOneMonster > 0) { //뱀피릭 터치
                ...
                if (totDamageToOneMonster > 0) { //트라이플링 윔
                    if (GameConstants.isWindBreaker(player.getJob()) && attack.skill != 13120003 && attack.skill != 13120010 && attack.skill != 13110022 && attack.skill != 13110027 && attack.skill != 13100022 && attack.skill != 13100027) {
                ...
                if (attack.skill == 1221011 || attack.skill == 21120006) { //생츄어리 or 콤보 템페스트
                ...
                if (attack.skill == 5011002) { //기간틱 백스탭
                    ...
                    if (eff.makeChanceResult()) {
                        monsterStatusEffect = new MonsterStatusEffect(Collections.singletonMap(MonsterStatus.SPEED, eff.getSkillStats().getStats("z")), SkillFactory.getSkill(5011002), null, false);
                    ...
                    if (!monster.getStats().isBoss()) {
                        if (oakid == 4) { //암흑 확률 상승
                            if (Randomizer.nextInt(100) < eff.getX()) {
                                monsterStatusEffect = new MonsterStatusEffect(Collections.singletonMap(MonsterStatus.DARKNESS, 1), SkillFactory.getSkill(5311004), null, false);
                                monster.applyStatus(player, monsterStatusEffect, false, eff.getSkillStats().getStats("v") * 1000 * 1000, false);
                            }
                        } else if (oakid == 3) { //슬로우 확률 상승
                            if (Randomizer.nextInt(100) < eff.getX()) {
                                monsterStatusEffect = new MonsterStatusEffect(Collections.singletonMap(MonsterStatus.SPEED, eff.getSkillStats().getStats("u")), SkillFactory.getSkill(5311004), null, false);
                                monster.applyStatus(player, monsterStatusEffect, false, eff.getSkillStats().getStats("v") * 1000, false);
                            }
                        } else if (oakid == 2) { //스턴 확률 상승
                            if (Randomizer.nextInt(100) < eff.getX()) {
                                monsterStatusEffect = new MonsterStatusEffect(Collections.singletonMap(MonsterStatus.STUN, 1), SkillFactory.getSkill(5311004), null, false);
                                monster.applyStatus(player, monsterStatusEffect, false, eff.getSkillStats().getStats("v") * 1000, false);
                            }
                        } else if (oakid == 1) { //결빙 확률 상승 (20%)
                            if (Randomizer.nextInt(100) < eff.getY()) {
                                monsterStatusEffect = new MonsterStatusEffect(Collections.singletonMap(MonsterStatus.FREEZE, 1), SkillFactory.getSkill(5311004), null, false);
                                monster.applyStatus(player, monsterStatusEffect, false, eff.getSkillStats().getStats("v") * 1000, false);
                            }
                        }
                    }
                ...
                if (player.getJob() == 511 || player.getJob() == 512 || player.getJob() == 1511) { //스턴 마스터리
                    if (player.getSkillLevel(5110000) > 0) {
                        ...
                        if (Randomizer.nextInt(100) < eff.getSkillStats().getStats("subProp")) {
                            monsterStatusEffect = new MonsterStatusEffect(Collections.singletonMap(MonsterStatus.STUN, 1), SkillFactory.getSkill(5110000), null, false);
                    ...
                    if (player.getSkillLevel(15110010) > 0) {
                        ...
                        if (Randomizer.nextInt(100) < eff.getSkillStats().getStats("subProp")) {
                            monsterStatusEffect = new MonsterStatusEffect(Collections.singletonMap(MonsterStatus.STUN, 1), SkillFactory.getSkill(15110010), null, false);
                            monster.applyStatus(player, monsterStatusEffect, false, 3000, false);
                        }
                    }
                    if (player.getSkillLevel(27101101) > 0) {
                        ...
                        if (Randomizer.nextInt(100) < eff.getSkillStats().getStats("subProp")) {
                            monsterStatusEffect = new MonsterStatusEffect(Collections.singletonMap(MonsterStatus.STUN, 1), SkillFactory.getSkill(27101101), null, false);
                            monster.applyStatus(player, monsterStatusEffect, false, 3000, false);
                        }
                    }
                }
                ...
                if (player.getBuffedValue(BuffStats.COMBO_DRAIN, 21101005) != null) {
                    ISkill skill = SkillFactory.getSkill(21101005);
                    player.addHP(Math.min(totDamage / 5, (totDamage * skill.getEffect(player.getSkillLevel(skill)).getX()) / 100));
                }
                if (player.getBuffedValue(BuffStats.COMBO_DRAIN, 1321054) != null) {
                    ISkill skill = SkillFactory.getSkill(1321054);
                    player.addHP(Math.min(totDamage / 5, (totDamage * skill.getEffect(player.getSkillLevel(skill)).getX()) / 100));
                }
                if (player.getBuffedValue(BuffStats.AURA_RECOVERY, 32101004) != null) {
                ...
                switch (attack.skill) {
                    ...
                    case 21101003: // 바디 프레셔
                    {
                        ...
                        if (player.getBuffedValue(BuffStats.AURA_RECOVERY) != null) {
                            ISkill skill = SkillFactory.getSkill(21100005);
                            player.addHP(Math.min(totDamage / 5, (totDamage * skill.getEffect(player.getSkillLevel(skill)).getX()) / 100));
                        }
                        if (attack.skill == 21100002) {
                            SkillStatEffect effzs = SkillFactory.getSkill(21100002).getEffect(player.getSkillLevel(21100002));
                            if (effzs.makeChanceResult() && !monster.getStats().isBoss()) {
                                monsterStatusEffect = new MonsterStatusEffect(Collections.singletonMap(MonsterStatus.STUN, 1), SkillFactory.getSkill(21100002), null, false);
                                monster.applyStatus(player, monsterStatusEffect, false, effzs.getDuration(), false);
                            }
                        }
                        if (attack.skill == 21120006) {
                            SkillStatEffect effzs = SkillFactory.getSkill(21120006).getEffect(player.getSkillLevel(21120006));
                            monsterStatusEffect = new MonsterStatusEffect(Collections.singletonMap(MonsterStatus.FREEZE, 1), SkillFactory.getSkill(21120006), null, false);
                            monster.applyStatus(player, monsterStatusEffect, false, effzs.getDuration(), false);
                        }
                        break;
                    }
                    default: //passives attack bonuses
                        if (totDamageToOneMonster > 0) {
                            if (player.getJob() == 121 || player.getJob() == 122) { // WHITEKNIGHT
                                if (player.getBuffedValue(BuffStats.WK_CHARGE) != null && attack.skill == 1211002) {
                                    ...
                                    if (chargedSkill.getEffect(player.getSkillLevel(charged)).makeChanceResult()) {
                                        ...
                                        if (charged == 1201012) { //블리자드 차지 -> 빙결
                                            mobEff = new MonsterStatusEffect(Collections.singletonMap(MonsterStatus.FREEZE, 1), chargedSkill, null, false);
                                            time = chargedSkill.getEffect(player.getSkillLevel(charged)).getY();
                                        }
                                        if (charged == 1201011) { //플레임 차지 -> 화염
                                            mobEff = new MonsterStatusEffect(Collections.singletonMap(MonsterStatus.POISON, chargedSkill.getEffect(player.getSkillLevel(charged)).getSkillStats().getStats("dot")), chargedSkill, null, false);
                                            time = chargedSkill.getEffect(player.getSkillLevel(charged)).getSkillStats().getStats("dotTime");
                                        }
                                        if (charged == 1211008) { //라이트닝 차지 -> 기절
                                            mobEff = new MonsterStatusEffect(Collections.singletonMap(MonsterStatus.STUN, 1), chargedSkill, null, false);
                                            time = chargedSkill.getEffect(player.getSkillLevel(charged)).getY();
                                        }
                                        if (charged == 1221004) { //디바인 차지 -> 봉인
                                            mobEff = new MonsterStatusEffect(Collections.singletonMap(MonsterStatus.SEAL, 1), chargedSkill, null, false);
                                            time = chargedSkill.getEffect(player.getSkillLevel(charged)).getY();
                                        }
                                ...
                                if (attack.skill == 1211002 && player.getOriginSkillLevel(1220010) > 0 && !monster.getStats().isBoss()) {
                                    SkillStatEffect effz = SkillFactory.getSkill(1220010).getEffect(player.getSkillLevel(1220010));
                                    if (effz.makeChanceResult()) { //어드밴스드 차지 즉사효과 발동!
                                        totDamageToOneMonster = 500000000;
                                    }
                                }
                            }
                        }
        ...
        if (effect != null) {
            if (attack.skill != 0 && attack.targets > 0 && attack.skill != 4331003 && attack.skill != 4341052) {
                if (attack.skill != 31121005 && effect.getSourceId() != 31011000 && effect.getSourceId() != 31201000 && effect.getSourceId() != 31211000 && effect.getSourceId() != 31221000) { //메타모포시스 재사용 방지
            ...
            if (monster != null) {
                ...
                for (Pair<Integer, Boolean> eachde : oned.attack) {
                    ...
                    if (eachd >= player.getStat().getMaxAttack() * 7.2f) {
                        ...
                        eachd = Randomizer.rand((int) (player.getStat().getMinAttack() * multiplier), (int) (player.getStat().getMaxAttack() * multiplier));
                    }
                    totDamageToOneMonster += eachd;
                }
                ...
                if (totDamageToOneMonster > 0) { 
                    ...
                    if (player.isEquilibrium()) {
                        if (GameConstants.isLightSkills(attack.skill)) {
                            player.addHP((long) Math.min((totDamageToOneMonster * (1 / 100.0D)), player.getStat().getCurrentMaxHp() / 2)); //체력 회복
                        }
                    }
                    ...
                    for (int i : venomskills) {
                        ...
                        if (player.getSkillLevel(skill) > 0) {
                            ...
                            if (venomEffect.makeChanceResult() && monster != null) {
                                monster.applyStatus(player, new MonsterStatusEffect(Collections.singletonMap(MonsterStatus.POISON, venomEffect.getSkillStats().getStats("dot")), SkillFactory.getSkill(i), null, false), true, venomEffect.getDuration(), false);
                        }
                            break;
                        }
                    }
                    ...
                    for (int i : skills) {
                        ...
                        if (player.getSkillLevel(skill) > 0) {
                            ...
                            monster.applyStatus(player, new MonsterStatusEffect(Collections.singletonMap(MonsterStatus.POISON, Integer.valueOf(1)), SkillFactory.getSkill(i), null, false), true, venomEffect.getDuration(), false);
                            break;
                        }
                    }
                    
                    if (player.getBuffedValue(BuffStats.AURA_RECOVERY, 32101004) != null) { //콤보 드레인
                        ISkill skill = SkillFactory.getSkill(32101004);
                        player.addHP((Math.min(totDamage / 5, (totDamage * skill.getEffect(player.getSkillLevel(skill)).getX()) / 100)));
                    }
                    if (eaterLevel > 0) {
    ...
    public static final AttackInfo Modify_AttackCrit(final AttackInfo attack, final MapleCharacter player, final int type, final SkillStatEffect effect) {
        if (attack.skill != 4211006 && attack.skill != 3211003 && attack.skill != 4111004) { //blizz + shadow meso + m.e no crits
            ...
            final boolean shadow = player.getBuffedValue(BuffStats.SHADOW_PARTNER) != null && (type == 1 || type == 2);
    ...
    public static AttackInfo parseDmgM(MapleCharacter chr, ReadingMaple lea, boolean touchAttack) {
        ...
        switch (ret.skill) {
            ...
            case 101110104:
            case 101120200:
                lea.skip(ret.skill == 11121055 || ret.skill == 14111022 || ret.skill == 14121004 || ret.skill == 14111023 ? 8 : ret.skill == 101000102 ? 1 : 4);
                break;
        }
        ret.display = lea.readShort();
        if (touchAttack) {// 바디프레셔, 텔마, 아수라, 미스트 스킬 등
            lea.skip(4);
        } else {
        ...
        lea.skip(ret.skill == 4210014 || ret.skill == 2100010 || ret.skill == 2201009 || (!touchAttack && ret.skill != 11121055) ? 8 : 4);
        lea.skip(ret.skill == 25111005 ? 4 : ret.skill == 14000028 || ret.skill == 14000029 || ret.skill == 14111022 || ret.skill == 14111023 
                || ret.skill == 14121004 || ret.skill == 14121052 ? 2 : GameConstants.isABYTE(ret.skill / 10000) || ret.skill == 14101021 ? 1 : 0);
        lea.skip(chr.getBuffedValue(BuffStats.SPIRIT_CLAW) != null ? 4 : 0);
    ...
    public static AttackInfo parseDmgMa(ReadingMaple lea, boolean Flame) {
        AttackInfo ret = new AttackInfo();
        lea.skip(!Flame ? 1 : 13);
        ret.tbyte = lea.readByte();
        ret.targets = (byte) ((ret.tbyte >>> 4) & 0xF);
        ...
        if (ret.skill == 22111011 || ret.skill == 22110023 || ret.skill == 22110023 || ret.skill == 22141011 
                || ret.skill == 22140022 || ret.skill == 22171062 || ret.skill == 22171063) {
            lea.skip(1);
        } else if (ret.skill == 22140024) {
            lea.skip(4);
        }
        ret.display = lea.readShort(); 
        lea.skip(5);
        ...
        for (int i = 0; i < ret.targets; i++) {
            ...
            /* damage 종료 */
            lea.skip(1); //1.2.252+
            if (ret.skill == 142120001 || ret.skill == 142110003) {
                lea.skip(8);
            }
     ...
     public static AttackInfo parseDmgR(MapleCharacter chr, ReadingMaple lea) {
        ...
        ret.unk = lea.readByte();
        ret.display = lea.readShort();
        lea.skip((ret.skill == 101000202) || (ret.skill == 101100202) || (ret.skill == 101120200) || (ret.skill == 101110204) 
                 ? 6 : (ret.skill == 5121017) || (ret.skill == 5310011) || (ret.skill == 5311010) ? 9 : (ret.skill == 3111013) || (ret.skill == 5221022) 
                 || (ret.skill == 95001000) ? 13 : (ret.skill == 23111001) || (ret.skill == 36111010) ? 17 : 5);
        ret.speed = lea.readByte(); // Confirmed
        ret.lastAttackTickCount = lea.readInt(); // Ticks
        ...
        lea.skip(chr.getBuffedValue(BuffStats.SPIRIT_CLAW) != null ? 12 : 8); //1.2.251+
