...
public class BBSHandler {
	private static final Logger logger = LoggerFactory.getLogger(BBSHandler.class);
	...
	private static void listBBSThreads(MapleClient c, int start) {
		try {
			...
			c.getSession().write(MainPacketCreator.BBSThreadList(rs, start));
	...
	private static void displayThread(final MapleClient c, final int threadid, final boolean bIsThreadIdLocal) {
		...
		try {
			...
			c.getSession().write(MainPacketCreator.showThread(bIsThreadIdLocal ? threadid : threadRS.getInt("localthreadid"), threadRS, repliesRS));

		} catch (SQLException se) {
			logger.debug("SQLException: {}", se);
		} catch (RuntimeException re) {
			logger.debug("The number of reply rows does not match the replycount in thread.  ThreadId = {}", re);
			try {
				ps = con.prepareStatement("DELETE FROM bbs_threads WHERE threadid = ?");
